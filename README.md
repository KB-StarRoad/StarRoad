...
### 🍿 AI 정책 상담 챗봇 (RAG)
등록된 청년정책·예적금 상품 데이터에서 근거를 찾아 답변하는 챗봇입니다. `/starroad/chat`

일반적인 챗봇은 모르는 것을 그럴듯하게 지어냅니다(환각). 금융 정책은 숫자 하나가 틀리면 
사용자가 실제로 손해를 보기 때문에, **모르면 모른다고 답하는 것**을 최우선으로 설계했습니다.
